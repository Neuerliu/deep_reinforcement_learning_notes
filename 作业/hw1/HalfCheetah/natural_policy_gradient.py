...
from mujoco_utils import moving_average, kl_divergence, flat_grad, flat_hessian, flat_params
...
class NaturalPolicyGradient:
    ...
    def update(self, transition_dict):
        ...
        step_dir = self.conjugate_gradient(state_tensor, loss_grad.data, 30, self.delta) # 计算梯度更新步长
...
if __name__ == '__main__':
    ...
    plt.xlabel("episode")
    plt.ylabel("return")
    plt.title("natural policy gradient on Half Cheetah")
    plt.show()
    
    mv_return = moving_average(return_list, 99)
    plt.plot(mv_return)
    plt.xlabel("episode")
    plt.ylabel("average return on last 100 episode")
    plt.title("natural policy gradient on Half Cheetah")
    plt.show()
