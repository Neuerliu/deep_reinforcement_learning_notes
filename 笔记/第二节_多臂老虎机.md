...
$$
现在我们将霍夫丁不等式运用于多臂老虎机问题中。将$\hat{Q}_t(a)$代入$\bar{x}_t$，令不等式中的参数$u=\hat{U}_t(a)$，代表不确定性度量。给定一个概率$p$，根据上述不等式，$Q_t(a)<\hat{Q}_t(a)+\hat{U}_t(a)$至少以概率$1-p$成立。

当$p$很小时，$Q_t(a)<\hat{Q}_t(a)+\hat{U}_t(a)$就以很大概率成立，$\hat{Q}_t(a)+\hat{U}_t(a)$便是期望奖励上界。此时，上置信界算法便选取**期望奖励上界最大的动作**，即$a=argmax_{a\in \mathcal{A}}[\hat{Q}_t(a)+\hat{U}_t(a)]$。
